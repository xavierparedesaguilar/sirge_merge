...
<section class="content-header">
    ...
    $this->Html->addCrumb($zabd->cod_zabd, ['controller' => 'Zabd', 'action' => 'edit', 'id' => $zabd->id]);
        ...
        <div class="col-xs-12 col-md-12 col-lg-12">
            <div class="box box-success">
                <div class="box-header with-border">
                    ...
                    ?>
                    <h3 class="box-title"> <strong> | Editar registro | </strong> <?php echo ($mod_modulo) ?></h3>
                ...
                <?php echo $this->Form->create($zabd, [
                    'url' => ['controller' => 'Zabd', 'action' => 'edit'],
                    'autocomplete' => "off",
                                            ...
                                            <div class="box box-success">
                                                ...
                                                <div class="box-body">
                                                    ...
                                                    <?php echo $this->Form->control('file_2', [
                                                        ...
                                                        // 'default' => empty($zabd->doc_resolucion)? '' : $zabd->doc_resolucion,
                                                        // 'value' => empty($zabd->doc_resolucion)? '' : $zabd->doc_resolucion,
                                                        'type' => 'file',
                                                    ]); ?>
...
<?php
if ($zabd->doc_resolucion) {
    $pdf = $this->Url->build('/', true) . $zabd->doc_resolucion;
} else {
    $pdf = '';
}
if ($zabd->mapa_img) {
    $imagen = $this->Url->build('/', true) . $zabd->mapa_img;
} else {
    $imagen = '';
}
if ($zabd->doc_georeferenciacion) {
    $xlsx = $this->Url->build('/', true) . $zabd->doc_georeferenciacion;
} else {
    $xlsx = '';
}
...
$this->Html->scriptBlock('$("#fec-reconocimientos").datepicker({autoclose: true,todayHighlight: true, language: "es", format: "dd-mm-yyyy", endDate: "0d"});', ['block' => true]);
...
$this->Html->scriptBlock('$("#file-1").fileinput({initialPreview: ["' . $imagen . '"], initialPreviewAsData: true,initialPreviewConfig: [{caption: "' . $zabd->mapa_img . '", showDelete: false}], overwriteInitial: true,  showUpload: false, language: "es", allowedFileExtensions: ["jpg", "png", "jpeg"] });', ['block' => 'script']);
$this->Html->scriptBlock('$("#file-2").fileinput({initialPreview: ["' . $pdf . '"], initialPreviewAsData: true,initialPreviewConfig: [{type:"pdf", caption: "' . $zabd->doc_resolucion . '", showDelete: false}], overwriteInitial: true, showUpload: false, language: "es", maxFileSize: 20480, allowedFileExtensions: ["pdf"] });', ['block' => 'script']);
$this->Html->scriptBlock('$("#file-3").fileinput({initialPreview: ["' . $xlsx . '"], initialPreviewAsData: true,initialPreviewConfig: [{caption: "' . $zabd->doc_georeferenciacion . '", showDelete: false}], overwriteInitial: true,  showUpload: false, language: "es", maxFileSize: 20480, allowedFileExtensions: ["xlsx","xls","xlsm","xlsm","xlsb","xltx"] });', ['block' => 'script']);
//************************ CK Editors ***********************//
