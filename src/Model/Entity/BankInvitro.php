...
class BankInvitro extends Entity
{
        ...
        protected function _getPropagacion()
    {
        ...
        protected function _getMedConservacion()
    {
        $result = TableRegistry::get('OptionList')->find()->where(['id' => $this->_properties['conservation']])->first();
    ...
    protected function _getPropagar()
    {
        $result = TableRegistry::get('Propagation_invitro')->find()->where(['bank_invitro_id' => $this->_properties['id']])->order(['id'=>'DESC'])->all();

        return $result;
    }

    protected function _getConservar()
    {
        $result = TableRegistry::get('Conservation_invitro')->find()->where(['bank_invitro_id' => $this->_properties['id'], 'status'=>'1'])->order(['id'=>'DESC'])->all();
