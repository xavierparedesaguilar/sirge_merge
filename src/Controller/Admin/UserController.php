            ...
            if ($this->request->is('post')) {
                ...
                try {
                    ...
                    if ($this->User->save($user)) {
                        ...
                        $this->Flash->success(__('La Información del Usuario '.$data['names'].' '.$data['surnames']. 'fue creado satisfactoriamente.'));
                    ...
                    $this->Flash->error(__('Hubo inconvenientes al crear usuario. Por favor, Otra vez intente-1.'));

                } catch (\Exception $e) {

                    $this->Flash->error(__('Complete los campos requeridos. Por favor, Otra vez intente-2.'));
                    return $this->redirect(['action' => 'index']);
                }
            }
        ...
        if($user_count==NULL && $this->permiso['edit']){
            ...
            if ($this->request->is(['patch', 'post', 'put'])) {
                ...
                if ($this->User->save($user)) {
                    ...
                    $this->Flash->success(__('La información del Usuario '.strtoupper($user->names) .' '.strtoupper($user->surnames).' fue actualizado satisfactoriamente.'));
