...
class FenotipicaFitoController extends AppController
{
    ...
    public function index()
    {
        ...
        if($this->permiso['index']){
            ...
            if ($this->request->is('post')) {
                ...
                $especie_nombre  = $this->Specie->find()->where(['collection_id' => $data['coleccion_id_'] , 'id' => $data['especie_id']])->first();
                $especie_nombre  = $especie_nombre->cropname;

                $value_specie    = $this->Specie->find()->where(['collection_id' => $data['coleccion_id_'] , 'id' => $data['especie_id']])->first();
                $value_specie    = $value_specie->genus.' '.$value_specie->species.' '.$value_specie->autor;
                
                $lista_especie = $this->Specie->find()->contain('Collection')->where(['Specie.collection_id' => $data['coleccion_id_'], 'Specie.id' =>  $data['especie_id']])->toArray();

                /*VERIFICACIÓN DE DESCRIPTORES SEGUN LA ESPECIE SELECCIONADA*/
                $count_descriptor    = $this->Descriptor->find()->where(['specie_id' => $data['especie_id'],'status' =>1])->all();      
                $count_descriptor    = $count_descriptor->count();

                 /*VERIFICACIÓN DE VALORES DE CARCATERFIZACION SEGUN LA ESPECIE SELECCIONADA*/

                $count_valdescrip    = $this->DescriptorValue->find()->contain('Descriptor')->where(['Descriptor.specie_id'=>$data['especie_id'],'Descriptor.status' =>1])->all(); 
                $count_valdescrip    = $count_valdescrip->count();   
                              
                $this->set(compact('mod_padre', 'mod_padre', 'mod_parent', 'colecciones', 'scripts', 'permiso','especie_id','especie_nombre','value_specie','lista_especie','count_descriptor','count_valdescrip','especie_idy','value_specie_idy','sql'));
                ...
                $sql="SELECT 
                SP.ID AS SPECIE_IDS, SP.GENUS AS SP_GENUS, SP.SPECIES AS SP_SPECIES, SP.AUTOR AS SP_AUTOR,SP.CROPNAME AS SP_CROPNAME,SP.FAMILY AS SP_FAMILY, SP.COLLECTION_ID AS COLLECTION_ID, COL.COLNAME AS COL_COLNAME
                FROM SPECIE SP
                INNER JOIN COLLECTION COL ON (COL.ID = SP.COLLECTION_ID)
                WHERE(SP.ID IN(SELECT DISTINCT(SPECIE_ID)FROM DESCRIPTOR WHERE STATUS=1 ORDER BY ID DESC))";
                $stmtData = $conn->prepare($sql);
                $stmtData->execute();

                $this->set(compact('mod_padre', 'mod_padre', 'mod_parent', 'colecciones', 'styles', 'scripts', 'permiso','sql','stmtData'));              
            }
    ...
    public function buscar( $idx = null , $idy = null )
    {
        ...
        $especies      = $this->Specie->find('list', ['keyField' => 'cropname',
                                                      'valueField' => function ($row) {
                                                                        return $row['genus']. ' ' .$row['species']. ' ' .$row['autor'];
                                                        }])->where(['collection_id' => $idx ])and ['id' => $especie_idx -> id];
        ...
        //$this->Flash->success(__('Lista de Descriptores de la Colección '. 'Specie.collection_id: '.  $idx. ' Specie.cropname: '.  $especie_idx->cropname  .' se importaron exitosamente.'));
    }
        ...
        if($this->permiso['import']){
                ...
                if ($this->request->is('post')) {
                    ...
                    if($data['form_tipo'] == 1){
                        ...
                        // $value_specie= 0;//$data['passport']['specie_id'];
                        ...
                        if(move_uploaded_file($data['file_carga']['tmp_name'], $fichero_subido)){
                            ...
                            try
                            {
                                ...
                                if($highestRow > 1){
                                    ...
                                    $header_excel = array('DESCRIPTOR', 'TITULO DEL DESCRIPTOR', 'DESCRIPCION');
                                    ...
                                    if($total_column == 3)
                                    {
                                        ...
                                        if($cont == $total_column){
                                            ...
                                            for ($row = 2; $row <= $highestRow; $row++){
                                                ...
                                                $temp_caract->especie = $model_specie->genus.' '.$model_specie->species.' '.$model_specie->autor;
                                            ...
                                            $this->Flash->success(__('Lista de Descriptores de la Colección '. $temp_caract->coleccion.' se importaron exitosamente.'));
    ...
    public function importarCaracterizacion()
    {
        if($this->permiso['import']){
                ...
                if ($this->request->is('post')) {
                    ...
                    $especie_idy = $this->Specie->find('list', ['keyField' => 'id',   'valueField' => function ($row) {
                                                                return $row['genus'] . ' ' . $row['species']. ' ' . $row['autor'];
                                                            }])
                                                            ->where(['collection_id' => $data['coleccion_id'] , 'status' => 1]);
                    ...
                    $especie_nombre_idy   = $this->Specie->find()->where(['collection_id' => $data['coleccion_id'] , 'id' => $data['especie_id']])->first();
                    ...
                    if(move_uploaded_file($data['file_caracterizacion']['tmp_name'], $fichero_subido)){
                        ...
                        try
                        {
                            ...
                            // echo"<pre>";
                            // var_dump($total_column.'-'.$total_columnas);exit;
                            // echo"</pre>";
                            ...
                            if($total_column == $total_columnas)
                            {
                                ...
                                for ($row = 2; $row < 3; $row++){
                                    ...
                                    for ($i=0; $i < $total_columnas; $i++) {
                                        ...
                                        if(strtoupper($columnas_final[$i]) == $rowData[0][$i]){
                                            $cont++;
                                        }
                                    }
                                }
                                        ...
                                        for ($row = 3; $row <= $highestRow; $row++){
                                            ...
                                            for ($i=3; $i < ($total_columnas-1); $i++) {
                                                ...
                                                if( ($rowData[0][0] != '' || $rowData[0][0] != NULL) && ($rowData[0][2] != '' || $rowData[0][2] != NULL) ){
                                                    ...
                                                    $temp_caract->descriptor_name   = strtoupper($columnas_final[$i]);
                ...
                $this->set(compact('descriptor', 'mod_padre', 'mod_parent', 'colecciones', 'scripts','especie_idy','value_specie_idy','especie_nombre_idy'));
                $this->set('_serialize', ['descriptor']);
