...
class BankSeedController extends AppController
{
    ...
    public function index()
    {
        ...
        if($this->permiso['index']){
            ...
            $sql="SELECT        
                S.ID AS id,S.LOTNUMB AS lotnumb,P.ACCENUMB AS accenumb,P.ACCNAME AS accname,
                P.OTHENUMB AS othernumb,C.COLNAME AS colname,E.CROPNAME AS cropname,S.ACQDATE AS acqdate,
                S.HARVESTDATE AS harvestdate,S.SEEWEIGHT AS seeweight, S.P1 AS p1,S.P2 AS p2,S.P3 AS p3,S.P4 AS p4,S.P5 AS p5,
                S.SEEDNUMB AS seednumb,S.N1 AS n1,S.N2 AS n2,S.N3 AS n3,S.N4 AS n4,S.N5 AS n5,S.BAGS AS bags,S.AVAILABILITY AS availability,
                S.ORIGIN AS origen
                FROM
                BANK_SEED S 
                INNER JOIN PASSPORT P ON (S.PASSPORT_ID=P.ID)
                INNER JOIN SPECIE E ON(E.ID = P.SPECIE_ID)
                INNER JOIN COLLECTION C ON(C.ID = E.COLLECTION_ID)
                WHERE (S.STATUS=1) ORDER BY S.LOTNUMB DESC";

            $bankSeed = $conn->prepare($sql);
            $bankSeed->execute();
    ...
    public function export()
    {
        ...
        if($this->permiso['export']){
            $filePath = WWW_ROOT .'pass_plantillas'. DS . 'formato_semilla.xlsx';
            $this->response->file($filePath , array('download'=> true));
    ...
    public function import()
    {
        ...
        $this->set(compact('modulo',  'scripts','permiso'));
    }
